...
class DefaultCompanyUserSeeder extends Seeder
{
    ...
    private function createUsers($company)
    {
        ...
        foreach ($roles as $roleData) {
            for ($i = 1; $i <= $roleData['count']; $i++) {
                $userData = $this->getUserData($roleData['role'], $isDemo, $i);
    ...
    private function getUserData($roleType, $isDemo, $index)
    {
        // If index > 1, use simple unique data to avoid duplicates
        if ($index > 1) {
            $name = 'User' . $index . '_' . substr(md5(uniqid()), 0, 6);
